...
async def seed_100_studios(db: AsyncSession) -> None:
    ...
    now = utc_now()
    studio_tz_result = await db.execute(select(Studio.id, Studio.timezone))
    studio_tz_map = {row.id: row.timezone for row in studio_tz_result.all()}
